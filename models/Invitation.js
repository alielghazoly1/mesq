...
const invitationSchema = new mongoose.Schema({
  ...
  // اسم اختياري بيحطه صاحب الدعوة عشان يميّز دعواته عن بعض في لوحته
  // (مثلاً "فرح أختي"). مالوش أي تأثير على شكل الدعوة اللي الضيف بيشوفها.
  // الدعوات القديمة مالهاش الحقل ده (فارغ) — بتظهر في اللوحة بأسماء العروسين
  // زي ما كانت بالظبط.
  name: { type: String, default: '', maxlength: 80 },

  // توكن سري وصعب تخمينه لصفحة الإحصائيات المشاركة (/s/:token). بيتولّد
  // للدعوات الجديدة وقت الإنشاء، وللدعوات القديمة أول ما صاحبها يطلب رابط
  // الإحصائيات من لوحته. مفيش default عن قصد: من غير قيمة الحقل بيفضل
  // **غير موجود** خالص في المستند، فالفهرس الـ sparse بيتخطّاه — وكده
